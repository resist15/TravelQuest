...
export default function AdventureDetails() {
  ...
  useEffect(() => {
    const fetchAdventure = async () => {
      try {
        const res = await axios.get(`/api/admin/adventures/${id}`);
  ...
  const handleSaveAdventureDetails = async () => {
    ...
    try {
      const res = await axios.put(`/api/admin/adventures/${currentState.id}`, formData, {
  ...
  const handleUploadNewImages = async () => {
    ...
    try {
      await axios.post(`/api/admin/adventures/${adventure.id}/images`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      const updatedAdventureRes = await axios.get(`/api/admin/adventures/${adventure.id}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/api/admin/adventures/${adventure.id}`);
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/admin/adventures/image/${adventure.id}?imageUrl=${encodeURIComponent(imageToDelete)}`);
